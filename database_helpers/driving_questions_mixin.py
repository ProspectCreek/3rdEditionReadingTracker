class DrivingQuestionsMixin:
    ...
    def get_driving_questions(self, reading_id, parent_id=None):
        # --- FIX: Added filter to exclude propositions ---
        sql = "SELECT * FROM reading_driving_questions WHERE reading_id = ? AND (type IS NULL OR type != 'proposition')"
        params = [reading_id]
    ...
    def get_driving_question_details(self, question_id):
        # --- FIX: Added filter to exclude propositions ---
        self.cursor.execute(
            "SELECT * FROM reading_driving_questions WHERE id = ? AND (type IS NULL OR type != 'proposition')",
            (question_id,)
        )
    ...
    def _next_driving_question_order(self, reading_id, parent_id):
        # --- FIX: Added filter to exclude propositions ---
        sql = "SELECT COALESCE(MAX(display_order), -1) FROM reading_driving_questions WHERE reading_id = ? AND (type IS NULL OR type != 'proposition')"
        params = [reading_id]
    ...
    def update_driving_question(self, question_id, data):
        ...
        self.cursor.execute("""
            UPDATE reading_driving_questions SET
                ...
                is_working_question = ?
            WHERE id = ? AND (type IS NULL OR type != 'proposition')
        """, (
    ...
    def delete_driving_question(self, question_id):
        # --- FIX: Added filter to exclude propositions ---
        self.cursor.execute(
            "DELETE FROM reading_driving_questions WHERE id = ? AND (type IS NULL OR type != 'proposition')",
            (question_id,)
        )
        self.conn.commit()
    ...
    def update_driving_question_order(self, ordered_ids):
        for order, q_id in enumerate(ordered_ids):
            # --- FIX: Added filter to exclude propositions ---
            self.cursor.execute(
                "UPDATE reading_driving_questions SET display_order = ? WHERE id = ? AND (type IS NULL OR type != 'proposition')",
                (order, q_id)
            )
        self.conn.commit()
    ...
    def find_current_working_question(self, reading_id):
        ...
        self.cursor.execute(
            """SELECT * FROM reading_driving_questions 
               WHERE reading_id = ? AND is_working_question = 1 AND (type IS NULL OR type != 'proposition')
               LIMIT 1""",
            (reading_id,)
        )
        return self._rowdict(self.cursor.fetchone())
    ...
    def clear_all_working_questions(self, reading_id):
        # --- FIX: Added filter to exclude propositions ---
        self.cursor.execute(
            "UPDATE reading_driving_questions SET is_working_question = 0 WHERE reading_id = ? AND (type IS NULL OR type != 'proposition')",
            (reading_id,)
        )
        self.conn.commit()